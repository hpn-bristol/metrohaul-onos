...
void terminal_device_ethernet_set_in_vlan(uint32_t lch, char const* const val) {
   printf("FPGA Driver - Index: %d - VLAN IN: %s\n", lch,val);
   char system_cmd[256];
...
void terminal_device_ethernet_set_out_vlan(uint32_t lch, char const* const val) {
   printf("FPGA Driver - Index: %d - VLAN OUT: %s\n", lch,val);
   char system_cmd[256];
...
// void terminal_device_ethernet_set_out_vlan(char const* const name, char const* const val) {
//     struct message_s mymsg;
//     mymsg.mtype = MESSAGE_TYPE;
//     sprintf(mymsg.mtext,"CONFIGOC###%s###OUTVLAN###%s$$$", name, val);
//     fprintf(stderr, "FPGA: %s\n", mymsg.mtext);
//  //    system("curl -X POST -k -u cumulus:CumulusLinux! -H \"Content-Type: application/json\" -d '{\"cmd\": \"show counters\"}' https://137.222.204.212:8080        /nclu/v1/rpc");
//      if (msgsnd(msg_queue_id, &mymsg, msg_size, IPC_NOWAIT) != 0) {
//           perror("failed out_vlan_set");
//          //maybe queue is full, check msgctl()
//      }
//  }
