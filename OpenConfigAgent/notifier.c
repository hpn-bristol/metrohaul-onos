...
static enum cdb_iter_ret Iter(confd_hkeypath_t *kp,
    ...
    //        len-1            -2          -3     -4    -5
    // /terminal-device/logical-channels/channel{$key}/logical-channel-assignments/...
    //        len-1            -2          -3     -4    -5
    // /components/component{$key}/optical-channel/config/...
    //    len-1       -2      -3       -4            -5
    
    fprintf(stderr,"ConfD operation: %d\n",operation); 
    switch (operation) {
        case MOP_CREATED: {
            fprintf(stderr, "\t **** MOP_CREATED (%d) KP Len: %d - buffer: %s", operation, kp->len, buf);
            // fprintf(stderr, "Creating %s\n", buf);

            // int tamanho = CONFD_GET_BUFSIZE(newv);
            // printf("\t\ttamanho: %d\n", tamanho);
            // printf("\t\tprefix: %s\n", prefix22);
            // printf("\t\tprefix_len: %d\n", prefix_len22);
            // printf("\t\tname: %s\n", name22);
            ...
            // printf("\t\t********* kp->len %s", kp->len);
            // if (kp->v[kp->len-11] != NULL) {
            //     // uint32_t vlan = CONFD_GET_UINT32(&kp->v[kp->len-11][0]);
            //     // fprintf(stderr, "\t\t It is not NULL! %d\n", vlan);
            //     fprintf(stderr, "\t\t It is not NULL!\n");
            //     printf("\t\t********* kp->len %d", kp->len);
            // } else {
        ...
        break;
        case MOP_MODIFIED: {
            fprintf(stderr, "Modifing %s\n", buf);
            fprintf(stderr, "\t **** MOP_MODIFIED (%d) KP Len: %d - buffer: %s", operation, kp->len, buf);
        }
        break;
        case MOP_VALUE_SET: {
            fprintf(stderr, "\t **** MOP_VALUE_SET (%d) KP Len: %d - buffer: %s", operation, kp->len, buf);
            ...
            // check if the root value is terminal-device
            if (strcmp(root_char,"terminal-device") == 0) {
                // get the logical channel index
                uint32_t lch_id = CONFD_GET_UINT32(&kp->v[kp->len-4][0]);
                fprintf(stderr, "Channel Index: %d\n", lch_id);

                /* The current possibilities of changes in terminal-device are:
                    - len-5 == oc_opt_term_config (config)
                    - len-5 == oc_opt_term_logical_channel_assignments (logical-channel-assignments)
                */
                confd_value_t *ctag = &kp->v[kp->len-5][0];
                switch (CONFD_GET_XMLTAG(ctag)) {
                    case oc_opt_term_config: {
                        /* The current possibilities of changes in terminal-device are:
                        - len-6 == oc_opt_term_admin_state (admin state)
                        */
                        ...
                        switch (CONFD_GET_XMLTAG(ctag)) {
                            ...
                            break;
                            default: {
                                /* Changes to this leaf does not reflect in any changes to Voyager */
                                fprintf(stderr, "Changes to leaf (%s) does not reflect in any changes to Voyager.\n", CONFD_GET_XMLTAG(ctag));
                            }
                            break;
                        }
                    }
                    break;
                    case oc_opt_term_logical_channel_assignments: {
                        /*  Detecting if a new Vlan is being added.
                            Vlans are located at the position 13 (kp->len-13).
                            /terminal-device/logical-channels/channel{$key}/..
                                len-1            -2             -3     -4     
                            /logical-channel-assignments/assignment{$key}/config/vlans/
                                    -5                      -6 		 -7    -8  	  -9
                            /vlan{$key}/config/vlan-id
                            - 10  -11    -12     -13  
                        */
                        // checking if the vlan is being modified.
                        if (kp->len == 13) {
                            // further checks:
                            // len-5 == oc_opt_term_logical_channel_assignments
                            // len-10 == oc_opt_term_vlan
                            confd_value_t *ctag_5 = &kp->v[kp->len-5][0];
                            confd_value_t *ctag_10 = &kp->v[kp->len-10][0];

                            if ((CONFD_GET_XMLTAG(ctag_5) == oc_opt_term_logical_channel_assignments)
                            && ((CONFD_GET_XMLTAG(ctag_10) == oc_opt_term_vlan))) {
                                // get the logical channel index (client port)
                                uint32_t lch_id = CONFD_GET_UINT32(&kp->v[kp->len-4][0]);
                                // fprintf(stderr, "Channel Index: %d\n", lch_id);

                                // get the logical channel assignment index (line port)
                                uint32_t lch_assignment_id = CONFD_GET_UINT32(&kp->v[kp->len-7][0]);
                                // fprintf(stderr, "Channel Assignment: %d\n", lch_assignment_id);

                                // get the vlan id
                                uint32_t vlan_id = CONFD_GET_UINT32(&kp->v[kp->len-11][0]);
                                // fprintf(stderr, "Vlan: %d\n", vlan_id);

                                // call voyager driver to add the vlan from the bridge
                                if (CONF_VOYAGER == 1) {
                                    char* result = voyagerSetVlan(lch_id, lch_assignment_id, vlan_id, "ADD");
                                    if (strcmp(result, "-1") == 0) {
                                        perror("Error during voyagerSetVlan.");
                                    }
                                    else {
                                        free(result);
                                    }
            ...
            else if(strcmp(root_char,"components") == 0) {
                ...
                /* The current possibilities of changes in terminal-device are:
                - len-6 == oc_opt_term_frequency (frequency)
                - len-6 == oc_opt_term_target_output_power (power) (not implemented yet)
                */
                confd_value_t *ctag = &kp->v[kp->len-6][0];
                switch (CONFD_GET_XMLTAG(ctag)) {
                    // frequency
                    case oc_opt_term_frequency: {
                        if (CONF_VOYAGER == 1) {
                            voyagerSetFrequency(name, newval);
                        }
                        struct copy *thr_args = malloc(sizeof(struct copy)); 
                        strcpy(thr_args->root, root_char);
        ...
        case MOP_DELETED: {
            ...
            fprintf(stderr, "\t **** MOP_DELETED (%d) KP Len: %d - buffer: %s", operation, kp->len, buf);

            // Check if root value is terminal-device
            if (strcmp(root_char,"terminal-device") == 0) {
                /*  This version can handle the deletion of vlans only.
                    Vlans are located at the position 13 (kp->len-13).
                    /terminal-device/logical-channels/channel{$key}/logical-channel-assignments/..
                        len-1            -2             -3     -4             -5
                    /assignment{$key}/config/vlans/vlan{$key}/config/vlan-id
                        -6 		 -7    -8  	  -9   -10  -11    -12     -13  
                */
                // checking if the vlan is being deleted.
                if (kp->len == 11) {
                    // further checks:
                    // len-5 == oc_opt_term_logical_channel_assignments
                    // len-10 == oc_opt_term_vlan
                    ...
                    if ((CONFD_GET_XMLTAG(ctag_5) == oc_opt_term_logical_channel_assignments)
                     && ((CONFD_GET_XMLTAG(ctag_10) == oc_opt_term_vlan))) {
                        ...
                        // call voyager driver to remove the vlan from the bridge
                        if (CONF_VOYAGER == 1) {
                            char* result = voyagerSetVlan(lch_id, lch_assignment_id, vlan_id, "DELETE");
                            if (strcmp(result, "-1") == 0) {
                                perror("Error during voyagerSetVlan.");
                            }
                            else {
...
int main(int argc, char **argv) {
    ...
    // // setup subscription point - logical channel assignments */
    // if ((cdb_subscribe(sub_sock, 3, oc_opt_term__ns, &spoint,
    //             "/terminal-device/logical-channels/channel/logical-channel-assignments/"))!= CONFD_OK) {
    //   confd_fatal("Terminate: subscribe \n");
    // }
    
    // setup subscription point - logical channel assignments */
    if ((cdb_subscribe(sub_sock, 3, oc_opt_term__ns, &spoint,
                "/terminal-device/logical-channels/channel/logical-channel-assignments/assignment/config/vlans/vlan/"))!= CONFD_OK) {
      confd_fatal("Terminate: subscribe \n");
    }
...
***            HPN-UoB             ***\n\n\n");
    ...
    // if(CONF_VOYAGER==1) initVoyagerHardwareCommunication();
    ...
    // if(CONF_TRANSPONDER == 1) stopHardwareCommunication();
    // if(CONF_VOYAGER == 1) stopVoyagerHardwareCommunication();
